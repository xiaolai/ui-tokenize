...
export function discoverConsumerProfile(root) {
  ...
  collectFiles(root, root, filesBySurface, ignore);
...
function collectFiles(dir, root, out, ignore) {
  let entries;
  try {
  ...
  for (const e of entries) {
    ...
    if (ignore.isIgnored(full)) continue;
    if (e.isDirectory()) {
      if (existsSync(join(full, 'package.json')) && full !== root) continue;
      collectFiles(full, root, out, ignore);
    } else if (e.isFile()) {
