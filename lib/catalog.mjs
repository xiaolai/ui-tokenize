...
export function isCatalogTokenSource(absPath, catalog) {
  return getCatalogSourceType(absPath, catalog) !== null;
}

/**
 * Returns the source type for a catalog token source path, or null when the
 * path isn't a token source. Used by the pre-tool-use hook to decide whether
 * direct Write/Edit needs the validated MCP path or can pass through:
 *
 *   - 'dtcg-json'  for tokens.json / design-tokens.json (any depth) — these
 *                  enforce DTCG schema, so direct edits would bypass schema
 *                  validation and naming-convention checks
 *   - the source.type from the discovered catalog otherwise — typically
 *                  'css-vars' or 'tailwind'; these are free-form sources
 *                  with no structural schema, so direct edits in maintainer
 *                  mode are safe and necessary for system migrations
 *
 * @param {string} absPath
 * @param {Catalog | null | undefined} catalog
 * @returns {string | null}
 */
export function getCatalogSourceType(absPath, catalog) {
  const norm = absPath.replace(/\\/g, '/');
  if (/(^|\/)(tokens|design-tokens)\.json$/.test(norm)) return 'dtcg-json';
  if (!catalog?.sources) return null;
  for (const s of catalog.sources) {
    const sourceAbs = (s.path.startsWith('/') ? s.path : join(catalog.root, s.path)).replace(/\\/g, '/');
    if (sourceAbs === norm) return s.type;
  }
  return null;
}
