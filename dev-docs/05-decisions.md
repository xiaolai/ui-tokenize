...
| D-008 | `token-reviewer` semantic-mis-pick subagent deferred to v0.2 | 2026-04-27 | resolved-in-v0.4 | v1 shipped the deterministic enforcement loop with `semantics-unchecked` labels (R-10). v0.2 / v0.3 shipped without the subagent (gap noted by external review). v0.4 ships `agents/token-reviewer.md` + `commands/cli.mjs review-prep` + `/tokenize:review` slash command — the deterministic finder collects catalog-resolved usages with surrounding context, the agent applies LLM judgment to classify each as correct / mis-pick / unclear |
